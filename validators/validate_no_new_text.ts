 ...
 */
function textExists(needle: string, haystack: string, method: 'EXACT' | 'NORMALIZED_EXACT'): boolean {
  if (method === 'EXACT') {
...
export function validateNoNewText(
  ...
  for (const change of proposedChanges) {
    ...
    if (source_reference.source_type === 'STANDARD_POSITION') {
      found = textExists(
        source_reference.exact_text,
        policySpec.standard_position.text,
        source_reference.match_method
      );
    } else if (source_reference.source_type === 'FALLBACK_ACCEPTABLE') {
      found = policySpec.fallback_acceptable_fragments.some(fragment =>
        textExists(source_reference.exact_text, fragment.text, source_reference.match_method)
      );
    }
