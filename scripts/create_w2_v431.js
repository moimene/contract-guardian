/**
 * CG-008.P: Create W2 v4.3.1 with Stability Patches
 * 
 * Fixes:
 * - Fix A: LiabilityLimitation with strong patterns
 * - Fix B: RightsReversion with termination context negatives
 * 
 * Run: node scripts/create_w2_v431.js
 */

const fs = require('fs');
const path = require('path');

const SOURCE_FILE = path.join(__dirname, '../n8n/wf operativos 0102_/W2_ClauseReview - Hybrid Router v4.3 (CG-008).json');
const OUTPUT_FILE = path.join(__dirname, '../n8n/wf operativos 0102_/W2_ClauseReview - Hybrid Router v4.3.1 (CG-008.P).json');

const workflow = JSON.parse(fs.readFileSync(SOURCE_FILE, 'utf8'));

console.log('CG-008.P: Creating W2 v4.3.1 with Stability Patches');
console.log('===================================================');

// Find the Hybrid Router Stage 1 node
const hybridRouter = workflow.nodes.find(n => n.name === 'Hybrid Router (Stage 1)');
if (hybridRouter) {
    // Complete updated router code with stability patches
    const updatedCode = `// CG-008.P: Hybrid Router Stage 1 with Stability Patches
...
// ============================================================
// CG-008.P: KEYWORD PATTERNS WITH STABILITY PATCHES
// ============================================================
...
const KEYWORD_PATTERNS = {
    ...
    // CG-008.P Fix A: LiabilityLimitation with STRONG patterns
    LiabilityLimitation: {
        patterns: [
            // Strong ALL CAPS patterns
    ...
    // CG-008.P Fix B: RightsReversion with termination context negatives
    RightsReversion: {
        patterns: [
    ...
    // CG-008: DataProtection with HIGH PRIORITY
    DataProtection: {
        patterns: [
            /\\bGDPR\\b/i,
            /\\bCCPA\\b/i,
    ...
    // CG-008: Confidentiality WITHOUT GDPR patterns
    Confidentiality: {
        patterns: [
    ...
    // CG-008: Assignment with expanded negatives
    Assignment: {
        patterns: [
    ...
    // TerminationConsequences with RightsReversion negatives
    ...
    TerminationRights: {
        patterns: [
            ...
            /terminate\\s+for\\s+convenience/i
        ],
        min_matches: 1,
        confidence: 0.88,
        priority: 2
...
return [{
    ...
    console.log('✓ Updated Hybrid Router Stage 1 with CG-008.P stability patches');
    console.log('  - Fix A: LiabilityLimitation strong patterns');
    console.log('  - Fix B: RightsReversion termination negatives');
}

// Update metadata
workflow.name = 'W2_ClauseReview - Hybrid Router v4.3.1 (CG-008.P)';
