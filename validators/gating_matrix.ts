 ...
 */
export function applyGatingMatrix(input: GatingInput): GatingOutput {
    const { required, routing_policy, final_status, confidence_overall, anchor_confidence, analysis_mode } = input;

    // Initialize result
    let decision: Decision = 'LOG_ONLY';
    let escalation_reason: EscalationReason | undefined;
    let escalation_queue: string | undefined;
 ...
 */
export function aggregateContractDecision(
    clauseDecisions: GatingOutput[]
): {
    ...
    const escalationsPending = clauseDecisions.filter(d =>
        d.decision === 'ESCALATE_HUMAN' && d.contract_level_flags.needs_human_review
    ).length;
