...
**Fecha**: 2026-01-20 (actualizado)
**Proyecto**: Contract Guardian / Amazon Redliner
**Estado**: ✅ COMPLETADO - Dataset expandido + UX propuesta
...
### Estadísticas del Dataset (Expandido con Harvey)

| Categoría | Base v1 | + Harvey | **Total** |
|-----------|---------|----------|-----------|
| **Materias** | 12 | +6 | **18** |
| **Clause Types** | ~50 | +33 | **~67** |
| **Policy Examples** | ~70 | +1,282 | **~1,352** |
| ├─ ACCEPTABLE | ~30 | +425 | ~455 |
| ├─ PASSABLE | ~15 | +432 | ~447 |
| └─ UNACCEPTABLE | ~25 | +425 | ~450 |
| **Fallback Clauses** | ~50 | - | **~50** |

### Dataset Harvey (Expansión)
**Ubicación**: `/AMAZON REDLINER/Dataset /`

| Archivo | Contenido |
|---------|-----------|
| `20260120060000_3layer_taxonomy_extension_harvey.sql` | 6 nuevas materias + 33 nuevos clause_types |
| `20260120061000_seed_harvey_policy_examples_expanded.sql` | 1,282 policy_examples adicionales |
| `harvey_policy_examples_db_ready.jsonl` | Dataset fuente procesado |
...
| `UX_EVOLUCION_3_CAPAS.md` | **NUEVO** Propuesta UX: roles, páginas, sidebar, grounding, feedback loop |
...
3. ✅ Expandir dataset RAG con Harvey (1,282 ejemplos)
4. 🔲 Implementar resolver `contract_type_review_defaults` en Edge Function
5. 🔲 Actualizar `n8nService.ts` para usar nuevo resolver

### Corto Plazo (Sprint 2-3) - UX Evolution
6. 🔲 **Fase 1 UX**: Nuevos roles + Sidebar dinámico (ver `UX_EVOLUCION_3_CAPAS.md`)
7. 🔲 **Fase 2 UX**: Páginas Blueprint Admin
8. 🔲 Implementar RAG retriever en pipeline n8n
9. 🔲 Actualizar Valuator agent para grounding con `policy_examples`
10. 🔲 Actualizar ChangeSet agent para usar `fallback_clauses`

### Medio Plazo (Sprint 4-6)
11. 🔲 **Fase 5 UX**: ContractReview con grounding visible
12. 🔲 **Fase 6 UX**: Feedback Loop
13. 🔲 Migrar UI de `clause_reviews` a `review_findings`
14. 🔲 Construir GraphRAG completo
15. 🔲 Export DOCX con redlines desde `review_findings`
...
- `20260120060000_3layer_taxonomy_extension_harvey.sql` **NUEVO**
...
- `20260120061000_seed_harvey_policy_examples_expanded.sql` **NUEVO** (1.1MB, 1,282 ejemplos)
...
- **Evolución UX (roles, páginas, sidebar)**: Ver `UX_EVOLUCION_3_CAPAS.md`

---

## 9. Nuevos Roles Propuestos (ver UX doc)

| Rol | Descripción | Páginas Principales |
|-----|-------------|---------------------|
| `client` | Usuario final, sube contratos | Dashboard, NewAnalysis, Review |
| `legal_ops` | Revisor escalaciones | Escalaciones, Métricas |
| `blueprint_admin` | Gestiona posiciones RAG | Blueprints, Matters, Policy Examples |
| `model_admin` | Gestiona templates | Contract Models, Defaults |
| `super_admin` | Administrador total | Todo + Users + Analytics |
