...
### 3. ACTUALIZAR UI/UX (NUEVO REQUISITO)

El usuario requiere una interfaz **guiada** y **educativa** para usuarios no expertos.

#### 3.1 Nueva Navegación (AppSidebar.tsx)
Añadir sección de "Configuración" (solo visible para admin/dev):
- **Dashboard**: Vista principal.
- **Nuevo Análisis**: Proceso guiado.
- **Escalaciones**: Gestión de bloqueos.
- **Configuración** (Nuevo):
  - **Playbooks**: Ver reglas activas.
  - **Knowledge Graph**: Visualizar grafo (placeholder por ahora).

#### 3.2 UX Guiada (Global)
- Añadir **Tooltips** (`<Tooltip>`) en todos los conceptos técnicos (ej. "RAG Status", "Embeddings", "Taxonomy").
- Añadir **Botones de Info** (`<Info className="h-4 w-4" />`) al lado de títulos de sección.
- Mensajes de ayuda "Inline" que expliquen qué está pasando en cada paso (ej. "Estamos analizando tu contrato contra el Playbook de Amazon DSA...").

### 4. ACTUALIZAR NewAnalysis.tsx
